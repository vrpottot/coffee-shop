...
export const SearchInput = ({
  ...
  const handleCountChange = (newAdults: number, newChildren: number) => {
  ...
  const getTotalPeople = () => {
    ...
    const word = getPersonWord(total);
    return `${total} ${word}`;
  };
      ...
      <div className="searchInputForm">
        ...
        <div className="filters">
          <SearchFilterButton onClick={openCalendar} className="whenButton">
            {selectedDate ? (
              <>
                <img src="/assets/icon/Дата.svg" alt="Дата" width="24" height="24" />
                <span style={{ color: 'var(--Black-300, #111)' }}>{selectedDate}</span>
              </>
            ) : (
              <>
                <img src="/assets/icon/Дата.svg" alt="Календарь" width="26" height="26" />
                <span>Когда?</span>
              </>
            )}
          </SearchFilterButton>
          ...
          <SearchFilterButton onClick={openCounter} className="whenButton">
            <img src="/assets/icon/Два человека.svg" alt="Люди" width="28" height="24" />
            <span style={{ width: '75px' }}>{getTotalPeople() || ' Кто едет?'}</span>
          </SearchFilterButton>
          {isCounterOpen && (
